...
- The Dockerfile at `images/headlessx/Dockerfile` builds HeadlessX from source using the upstream repository at
...
- The build uses a three-stage Dockerfile pattern matching the official HeadlessX Dockerfile:
  ...
  3. **Runtime stage**: Uses the official Playwright base image (`mcr.microsoft.com/playwright:v1.56.0-noble`) with all
     browser dependencies pre-installed
- The CI workflow in `.github/workflows/image-build.yaml` automatically builds and pushes the image to
  `ghcr.io/theepicsaxguy/headlessx:1.2.0` when changes are made to the Dockerfile.
...
- `AUTH_TOKEN` comes from `es-headlessx-auth-token`, which pulls `app-headlessx-auth-token` out of Bitwarden through
  External Secrets.
...
- Pod security: `runAsNonRoot`, `runAsUser`/`runAsGroup` 1000, default seccomp, no privilege escalation, and all
  capabilities dropped.

## Networking

- Service `headlessx` exposes port 3000 internally.
- `HTTPRoute` binds `headlessx.peekoff.com` to the `internal` Gateway and forwards all paths to the Service.
- `NetworkPolicy` only permits ingress from namespaces labeled `name=gateway`, and limits egress to DNS plus outbound
  HTTP/HTTPS.
- Publish `headlessx.peekoff.com` inside the private DNS zone so the Gateway’s internal address resolves for callers.
...
3. Resolve `headlessx.peekoff.com` from an internal network and curl through the Gateway → HTTP 200.
4. Inspect `/app/logs` to verify log files reach the PVC. Tune retention or ship to centralized logging before the
   volume fills.
