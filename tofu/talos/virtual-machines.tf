# VMs using the primary Proxmox provider
resource "proxmox_virtual_environment_vm" "primary" {
  for_each = {
    for k, v in local.internal_nodes : k => v
    if var.proxmox_secondary == null || v.host_node != var.proxmox_secondary.name
  }
...
# VMs using the secondary Proxmox provider
resource "proxmox_virtual_environment_vm" "secondary" {
